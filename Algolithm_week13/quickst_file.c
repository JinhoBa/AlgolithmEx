...
main(int argc, char* argv[])
{
	...
	while (fscanf(infile, "%ld %s %d %s", &(edb[i].eid), edb[i].name,
		&(edb[i].escore), edb[i].dname) != EOF)
		i++;
	...
	printf("\n\nSorted Data .... \n");
	for (j = 0; j < i; j++)
		fprintf(out, "%ld\t%s\t%d\t%s\n", (edb[j].eid), edb[j].name,
			(edb[j].escore), edb[j].dname);
